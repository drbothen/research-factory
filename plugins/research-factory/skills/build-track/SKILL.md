...
2. **Draft (researcher).** Dispatch the `researcher` agent to gather sources and draft/advance the track's L1/L2 observations, honoring the per-track sourcing rule. Every claim gets a citation or an explicit flag. **For a NEW track, scaffold its doc set from `${CLAUDE_PLUGIN_ROOT}/templates/corpus/`** (`L2-baseline`, `L2-baseline-tldr`, `L3-findings`, `L3-findings-tldr`, `track-summary`) — substitute the placeholders (`<market>`, `<track-slug>`, vector rows from `factory.config.yaml`) and strip the guidance comments. (L3 synthesis with the `synthesizer` agent arrives at v0.5; in v0.1 produce the sourced L1/L2 base.)
